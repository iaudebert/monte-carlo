...
def render_terminal_v3(ticker, n_sims, forecast_days, zoom_months, calc_months, ma_period, interval, show_ma, mode="FULL"):
    ...
    try:
        ...
        # 3. MOTEUR QUANT (GARCH + CUDA)
        do_sim = (mode == "FULL")
        prob_profit, model_info = 0, ""
        p1, p5, p50, p95, p99 = [None]*5
        ...
        if do_sim and len(prices_calc) > 50:
            ...
            # Simulation CUDA
            mu_gpu = cp.float32(mu_annual / 365)
            sigma_gpu = cp.float32(sigma_annual / np.sqrt(365))
            ...
            res_gpu = cp.percentile(price_paths_gpu, [1, 5, 50, 95, 99], axis=1)
            p1, p5, p50, p95, p99 = [cp.asnumpy(res_gpu[i]) for i in range(5)]
            prob_profit = float(cp.mean(price_paths_gpu[-1, :] > last_price) * 100)
        ...
        # AXE 1 : ZOOM & SIMULATION
        ax.plot(df[df.index >= zoom_start].index, df[df.index >= zoom_start]['Close'], color='white', alpha=0.1 if do_sim else 0.4, lw=1)
        
        if len(prices_calc) > 10:
            X_plot = np.arange(len(prices_calc)).reshape(-1, 1)
            trend_line = reg.predict(X_plot).flatten()
            ax.plot(df_calc.index, trend_line, color='white', lw=1.5, alpha=0.5, label="Trend Hist.")
            ...
            ax.plot(future_dates, reg.predict(future_x_reg).flatten(), color='white', lw=2, ls='--', alpha=0.4)

        if show_ma and 'MA' in df.columns:
            ax.plot(df[df.index >= zoom_start].index, df['MA'].loc[df.index >= zoom_start], color='cyan', lw=1.2, alpha=0.6)

        if do_sim and p1 is not None:
            future_dates_num = mdates.date2num(future_dates)
            ax.hist2d(np.repeat(future_dates_num, 4000), cp.asnumpy(price_paths_gpu[:, :4000]).flatten(), bins=[forecast_days, 100], cmap='gray', alpha=0.4, cmin=1)
            ax.plot(future_dates, p99, '#00FF00', lw=1.2, alpha=0.8, label="Top 1%")
            ax.plot(future_dates, p50, 'orange', lw=2, label="Médiane GARCH")
            ax.plot(future_dates, p1, '#FF0000', lw=1.2, alpha=0.8, label="Flop 1%")
            ax.set_ylim(p1[-1]*0.8, p99[-1]*1.2)
            ax.set_xlim(mdates.date2num(zoom_start), future_dates_num[-1])

        ax.set_title(f"TERMINAL QUANT REALISTE | {ticker} | Profit: {prob_profit:.0f}% | {model_info}", loc='left', fontsize=12)
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%d/%m' if interval != "1d" else '%m/%y'))

        # AXE 2 : HISTOGRAMME MIROIR
        if do_sim and p1 is not None:
            final_prices = cp.asnumpy(price_paths_gpu[-1, :])
            n, bins, patches = ax2.hist(final_prices, bins=120, orientation='horizontal', density=True, alpha=0.1)
            for i in range(len(patches)):
                ...
                elif bc < p1[-1]: patches[i].set_facecolor('#FF0000'); patches[i].set_alpha(0.7)
                elif bc < p5[-1]: patches[i].set_facecolor('#FF6600'); patches[i].set_alpha(0.5)
            
            ax2.invert_xaxis()
            ax2.axhline(last_price, color='cyan', lw=2, alpha=0.8)
            ax2.text(0.98, last_price, f"{last_price:,.2f}", transform=ax2.get_yaxis_transform(), color='cyan', ha='right', va='bottom', fontweight='bold')
            ax2.text(0.98, p99[-1], f"P99: {p99[-1]:,.0f}", transform=ax2.get_yaxis_transform(), color='#00FF00', ha='right', fontsize=8)
            ax2.text(0.98, p1[-1], f"P1: {p1[-1]:,.0f}", transform=ax2.get_yaxis_transform(), color='#FF0000', ha='right', fontsize=8)
        ...
        if len(prices_calc) > 10:
            ax3.plot(df_calc.index, trend_line, color='white', lw=1.5, alpha=0.6)
        ax3.axvspan(calc_start, last_date, color='yellow', alpha=0.05)
